...
function woocommercetags($tags,$access,$id=null,$country=null,$operation=null){
    set_time_limit(0);    
    $woocommerce = createWoo($access);
    $updateListTags=[];
    foreach($tags as $tag){    
...
function woocommerceproductsrc($products,$access,$id=null,$country=null,$operation="update"){
    ...
    $products=shopSkus($products,['2GU604P2']);
    ...
    foreach($products as $product){
            ...
            //if(is_null($product['ProductWeight'])){
                    ...
                    $rcSet =$functionCallBack($product['sku']);
...
function woocommerceproducts($products,$access,$id=null,$country=null,$operation="create"){     
    ...
    if(in_array($operation,['create'])){$products=shopIdIsNull($products); $products=shopSkus($products,['2GU604P2']);}
    $x=0;
...
function getShopId($country,$id,$type,$user_id){
    ...
    if($type=='manufacturer'){
	    $manufacturer = $db->table('manufacturer_'.$country.'_test')->where(['id' => $id])->get()->getResultArray();
        $manufacturer[0]['shop_id']=null;
        return ["shop_id"=>array_values(woocommercemanufacturer($manufacturer,$access_,$user_id,$country)['tags'])];
    }
    if($type=='category'){
        $category = $db->table('categorias_'.$country.'_test')->where(['id' => $id, 'type' => 'category-product'])->get()->getResultArray();
        $category_[]= ["parent"=>[
            "shop_id"=> null,
            "id"=> $category[0]['id'],
            "parent_id"=> 0,
    ...
    if($type=='subcategory'){
        $subCategory = $db->table('categorias_'.$country.'_test')->where(['id' => $id, 'type' => 'sub-product'])->get()->getResultArray();
        $subCategory_[]= ["child"=>[[
            "shop_id"=> null,
            "id"=> $subCategory[0]['id'],
    ...
    if($type=='tags'){
        $productLine = $db->table('categorias_'.$country.'_test')->where(['id' =>$id, 'type' => 'line-product'])->get()->getResultArray();        
...
function setIdShop($country,$id,$catalog,$type,$user_id=null,$keyLabel=null,$idLabel='shop_id'){
    ...
    if(count($search_item)<=0){       
        $search_item[] = getShopId($country,$id,$type,$user_id);
