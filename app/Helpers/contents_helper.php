		...
		if(file_exists('./getContents/ca/'.$item['sku'].'.json')){
			...
			if(empty($manufacturer) && (!empty($product['productDetail']['vendor']) && !is_null($product['productDetail']['vendor']))){
				...
				if ($db->transStatus() === FALSE) {
					...
					$db->table('manufacturer_ca')->update(['id_marca_ingram' => $manufacturer[0]['id']],["id"=>$manufacturer[0]['id']]);
			...
			if(empty($category) &&  (!empty($product['productDetail']['category']) && !is_null($product['productDetail']['category']))){
				...
				if ($db->transStatus() === FALSE) {
					...
					$db->table('categorias_ca')->update(['id_categoria_ingram' => $category[0]['id']], ['id' => $category[0]['id']]);
			...
			if(empty($subCategory)&&(!empty($product['productDetail']['subCategory'])&&!is_null($product['productDetail']['subCategory']))){
				...
				if ($db->transStatus() === FALSE) {
					...
					$db->table('categorias_ca')->update(['id_categoria_ingram' => $subCategory[0]['id']], ['id' => $subCategory[0]['id']]);
			...
			if(empty($productLine)&&(!empty($product['productDetail']['productLine'])&&!is_null($product['productDetail']['productLine']))){
				...
				if ($db->transStatus() === FALSE) {
					...
					$db->table('categorias_ca')->update(['id_categoria_ingram' => $productLine[0]['id']], ['id' => $productLine[0]['id']]);
			...
			$id_category = (isset($category[0]['id_categoria_ingram'])?$category[0]['id_categoria_ingram']:null);

			$id_subcategory = (isset($subCategory[0]['id_categoria_ingram'])?$subCategory[0]['id_categoria_ingram']:null);

			$id_line = (isset($productLine[0]['id_categoria_ingram'])?$productLine[0]['id_categoria_ingram']:null);

			$id_manufacturer = (isset($manufacturer[0]['id_marca_ingram'])?$manufacturer[0]['id_marca_ingram']:null);
			...
			if (file_exists('./getContents/mx/' . $item['sku'] . '.json')) {
				...
				if (empty($manufacturer) && (!empty($product['productDetail']['vendor']) && !is_null($product['productDetail']['vendor']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						$db->table('manufacturer_mx')->update(['id_marca_ingram' => $manufacturer[0]['id']], ["id" => $manufacturer[0]['id']]);
				...
				if (empty($category) &&  (!empty($product['productDetail']['category']) && !is_null($product['productDetail']['category']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						//$db->table('categorias_mx')->update(['id_categoria_ingram' => $category[0]['id']], ['id' => $category[0]['id']]);
				...
				if (empty($subCategory) && (!empty($product['productDetail']['subCategory']) && !is_null($product['productDetail']['subCategory']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						//$db->table('categorias_mx')->update(['id_categoria_ingram' => $subCategory[0]['id']], ['id' => $subCategory[0]['id']]);
				...
				if (empty($productLine) && (!empty($product['productDetail']['productLine']) && !is_null($product['productDetail']['productLine']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						$db->table('categorias_mx')->update(['id_categoria_ingram' => $productLine[0]['id']], ['id' => $productLine[0]['id']]);
...
function RCco($sku = null, $reset = false)
{
	...
	try {
		...
		if (!$reset) {
			...
			if (!is_null($sku)) {
				...
				$skus = $db->table('products_co')->select('sku')->get()->getResultArray();
			}
		}
		...
		foreach ($skus as $item) {
			...
			if (file_exists('./getContents/co/' . $item['sku'] . '.json')) {



				$product = json_decode(file_get_contents('./getContents/co/' . $item['sku'] . '.json'), true);
				...
				if (
					...
					$productFail = (file_exists('./getContents/co_product_json_fail.json')) ? json_decode(file_get_contents('./getContents/co_product_json_fail.json'), true) : [];

					$productFail[] = $item['sku'];

					file_put_contents('./getContents/co_product_json_fail.json', json_encode(array_unique($productFail)));
				...
				$category = $db->table('categorias_co')->where(['name' => $product['productDetail']['category'], 'type' => 'category-product'])->get()->getResultArray();

				$subCategory = $db->table('categorias_co')->where(['name' => $product['productDetail']['subCategory'], 'type' => 'sub-product'])->get()->getResultArray();

				$productLine = $db->table('categorias_co')->where(['name' => $product['productDetail']['productLine'], 'type' => 'line-product'])->get()->getResultArray();

				$manufacturer = $db->table('manufacturer_co')->where(['manufacturer' => $product['productDetail']['vendor']])->get()->getResultArray();



				if (empty($manufacturer) && (!empty($product['productDetail']['vendor']) && !is_null($product['productDetail']['vendor']))) {

					$db->table('manufacturer_co')->insert([

						'manufacturer' => $product['productDetail']['vendor'],

						'slug' => slugify($product['productDetail']['vendor']),

						'created_at' => returnDate(returnAccess()['zone']['co'], "Y-m-d H:i")
					]);
					...
					if ($db->transStatus() === FALSE) {
						...
						$db->table('manufacturer_co')->update(['id_marca_ingram' => $manufacturer[0]['id']], ["id" => $manufacturer[0]['id']]);
				...
				if (empty($category) &&  (!empty($product['productDetail']['category']) && !is_null($product['productDetail']['category']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						//$db->table('categorias_co')->update(['id_categoria_ingram' => $category[0]['id']], ['id' => $category[0]['id']]);

						$db->table('categories_co')->insert([
				...
				if (empty($subCategory) && (!empty($product['productDetail']['subCategory']) && !is_null($product['productDetail']['subCategory']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						//$db->table('categorias_co')->update(['id_categoria_ingram' => $subCategory[0]['id']], ['id' => $subCategory[0]['id']]);
				...
				if (empty($productLine) && (!empty($product['productDetail']['productLine']) && !is_null($product['productDetail']['productLine']))) {
					...
					if ($db->transStatus() === FALSE) {
						...
						$db->table('categorias_co')->update(['id_categoria_ingram' => $productLine[0]['id']], ['id' => $productLine[0]['id']]);
				...
				$db->table('products_co')->update(['id_manufacturer' => $id_manufacturer, 'id_category_parent' => $id_category, 'id_category' => $id_subcategory, 'id_line' => $id_line], ['sku' => $item['sku']]);
				...
				if ($db->transStatus() === FALSE) {
					...
					if ($reset) {
						...
						extractContents($item['sku'], 'co', $object_p);
					}
				}
...
function extractContentsBDI($sku, $country, $object_p = null, $access=null)
{
	
	$object_p_reponse = bdi($access, $sku);

	

	foreach($object_p_reponse as $object_p){
		...
		if ($object_p) {
			...
			write_file_root('./getContents/' . $country .'/' , $object_p->productDetail->sku . '.json', json_encode($object_p, JSON_UNESCAPED_UNICODE));
			...
			if (!is_null($object_p) && isset($object_p->productDetail->productImage)) {
				$contentsImages = ["imageGalleryUrlHigh" => (isset($object_p->productDetail->productImage->imageGalleryUrlHigh)) ? explode(",", $object_p->productDetail->productImage->imageGalleryUrlHigh) : [], "images" => $object_p->productDetail->productImage];
			}
			$vpn=null;
			if (!is_null($object_p) && isset($object_p->productDetail->title)) {
				$vpn = isset($object_p->productDetail->vpn) ? $object_p->productDetail->vpn : '';
				$vendor = isset($object_p->productDetail->vendor) ? $object_p->productDetail->vendor : '';
				$contentsTitle = $vendor . " " . $object_p->productDetail->title . " " . $vpn/*." ". $object_p->productDetail->description*/;
			...
			$ProductWeight = (!is_null($object_p->productDetail->productMeasurement->pMeasureWeight) && !empty($object_p->productDetail->productMeasurement->pMeasureWeight)? number_format($object_p->productDetail->productMeasurement->pMeasureWeight, 2, '.', '') : null);

			$ProductLength = (!is_null($object_p->productDetail->productMeasurement->pMeasureLength)&& !empty($object_p->productDetail->productMeasurement->pMeasureLength)? number_format($object_p->productDetail->productMeasurement->pMeasureLength, 2, '.', '') : null);

			$ProductWidth = (!is_null($object_p->productDetail->productMeasurement->pMeasureWidth) && !empty($object_p->productDetail->productMeasurement->pMeasureWidth) ? number_format($object_p->productDetail->productMeasurement->pMeasureWidth, 2, '.', '') : null);

			$ProductHeight = (!is_null($object_p->productDetail->productMeasurement->pMeasureHeight)&& !empty($object_p->productDetail->productMeasurement->pMeasureHeight) ? number_format($object_p->productDetail->productMeasurement->pMeasureHeight, 2, '.', '') : null);
...
function extractContents($sku, $country, $object_p = null, $access = null)
{
	...
	if (is_null($object_p)) {
		...
		//if (!$object_p) {
			...
			if (!is_null($access)) {
				...
				$object_p_ = bdi($access, $sku);

				if (isset($object_p_[0])) {
					$object_p = $object_p_[0];
					if (!is_null($object_p->productDetail->subCategory) && !empty($object_p->productDetail->subCategory)) {
						write_file_root('./getContents/' . $country . '/', $object_p->productDetail->sku . '.json', json_encode($object_p, JSON_UNESCAPED_UNICODE));
					} else {
						$object_p = null;
					}
		...
		if (!$object_p) {
			...
			if (!is_null($access)) {
				...
				if (isset($object_p_[0])) {
					$object_p = $object_p_[0];
					if (!is_null($object_p->productDetail->subCategory) && !empty($object_p->productDetail->category)) {
						write_file_root('./getContents/' . $country . '/', $object_p->productDetail->sku . '.json', json_encode($object_p, JSON_UNESCAPED_UNICODE));
					} else {
						$object_p = null;
					}
...
function extractContentsBatch($skusBDI,$country,$object_p = null, $access=null){
	if(is_array($skusBDI)){
		

		try{
			$skusExtract = implode(',', $skusBDI);
			$object_p_ = bdi($access, $skusExtract);
			if(!empty($object_p_)){

				

				foreach($object_p_ as $object_p){

					if (!isset($object_p->productDetail->sku) || empty($object_p->productDetail->sku)) {continue;}

					$sku= $object_p->productDetail->sku;
					...
					if (!isset($object_p->productDetail->subCategory) || !isset($object_p->productDetail->vendor)) {continue;}
					
					if (empty($object_p->productDetail->subCategory) || empty($object_p->productDetail->vendor)) {continue;}
					if (is_null($object_p->productDetail->subCategory) || is_null($object_p->productDetail->vendor)) {continue;}
				...
				/* var_dump($fileContents[$sku]);
						...
						//if(!file_exists('./getContents/' . $country . '/', $object_p->productDetail->sku . '.json')){
							write_file_root('./getContents/' . $country . '/', $object_p->productDetail->sku . '.json', json_encode($object_p, JSON_UNESCAPED_UNICODE));
						//}

						//$isProduct[$sku] = $fileContents[$sku];				
				}
			}
		} catch (Exception $e) {
	...
	return (isset($fileContents)? $fileContents:null);
}
