...
function opencartmanufacturer($manufacturer,$access,$id=null,$country=null,$operation=null, $skus = null, $update = 'all'){
    set_time_limit(0);    
    $opencart = createOpenCart($access);
...
function opencarttags($tags,$access,$id=null,$country=null,$operation=null, $skus = null, $update = 'all'){
    set_time_limit(0);    
    $opencart = createOpenCart($access);
    $updateListTags=[];
    foreach($tags as $tag){    
...
function opencartcategories($categories,$access,$id=null,$country=null,$operation=null, $skus = null, $update = 'all'){
    set_time_limit(0);
    $opencart = createOpenCart($access);
...
function opencartprices($products,$access,$id=null,$country=null,$operation=null, $skus = null, $update = 'all'){
    set_time_limit(0);
    ...
    $products = ($update == 'all') ? shopIdIsNotNull($products) : shopIdIsNotNullAndIsActive($products);

    if(!is_null($skus)){$products =shopSkus($products, $skus);}
    ...
    outInstance($country, $id); 
    return  ["success" => true, "msg" => "Price Update Finish"];
}
...
$parent_id_new=null;
$catalogs_ = null;
function opencartproductsrc($products,$access,$id=null,$country=null,$operation="update", $skus = null, $update = 'all'){
    global $access_;
...
function opencartcontents($products,$access,$id=null,$country=null,$operation="update", $skus = null, $update = 'all'){
    set_time_limit(0);
    ...
    //$products = shopIdIsNotUpdateAndContentsNull($products);
    //$products = shopIdIsNotUpdate($products);

    

    $products = shopSkus($products,[
        "5475466",
        "5501575",
        "5319157",
        "5501577",
        ...
        foreach ($productsSeg as $products_) {       
                ...
                foreach($products_ as $product){
                        ...
                        if (!is_null($product['title'])) {
                            $data_productos_['name'] = $product['title'];
                        }

                        if(!is_null($array_img)){
                        ...
                        if(isset($data_productos_['images'] ) || isset($data_productos_['description'])){
                            $data_productos[$operation][] = $data_productos_;
                            /* var_dump($data_productos);
                            exit; */
                        
                        }
                        //if(count($data_productos[$operation])>10){ echo count($data_productos[$operation]); var_dump($data_productos[$operation]);exit; }                   
                }                

                if(count($data_productos[$operation])>0){  

                    $object_response_data = $opencart->post('products/batch', $data_productos);
            /* echo json_encode($object_response_data);
            exit; */
                    //try{
                        if(count($object_response_data->update)>0){
                            foreach($object_response_data->update as $elem){      
    ...
    /* }catch (Exception $e) {
        ...
        write_file_root("getSegmentation/".$country.'/'.$id."/","log.json",json_encode($log,JSON_UNESCAPED_UNICODE));  
        outInstance($country, $id);
        echo $e->getMessage().json_encode($object_response_data);
        return $log;        
    } */
    outInstance($country, $id);
...
function opencartproducts($products,$access,$id=null,$country=null,$operation="create", $skus = null, $update = 'all'){
    ...
    foreach($productsSeg as $products_){
        ...
        try{
            ...
            foreach($productPrices as $product){ 
                    ...
                    if(!is_null($product['partNumber'])){
                        $data_productos_['model'] = $product['partNumber'];
                            ...
                            if($elem->id==0){     
                                if($product_id>0){    
                                    if(!isset($elem->sku)){                   
                                        $productDuplicate = $opencart->get('products/'.$product_id);                            
                                        $updateListProducts[$productDuplicate->sku]=$product_id;
                                    }else{$updateListProducts[$elem->sku] = $product_id;}
                                }
                            }else{
                                $updateListProducts[$elem->sku]=$elem->id;
                            }
...
function shopIdIsNotUpdateAndContentsNull($catalog)
{
    $search_item = array_filter(
        $catalog,
        function ($item) {
            if (isset($item['shop_id'])) {
                return (!isset($item['shop_update_at']) && (!is_null($item['ficha_html']) || !is_null($item['images'])));
            }
        }
    );
    return $search_item;
...
function shopIdIsNotUpdate($catalog)
{
    $search_item = array_filter(
        $catalog,
        function ($item) {
            if (isset($item['shop_id'])) {
                return !isset($item['shop_update_at']);
            }
        }
    );
    return $search_item;
