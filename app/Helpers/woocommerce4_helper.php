    ...
    foreach($manufacturer as $brand){    
        ...
        if(is_null($brand['shop_id'])){    
            ...
            try{
                ...
                $bandGet =  $woocommerce->get( 'brands' );
                
                $brandObj = array_filter(
                    $bandGet,
                    function($item) use($errorBrand){
                        return ($item->name == $errorBrand['name']);
                    }
                );
                $updateListManufacturer["brands"][$brand['id']] = reset($brandObj)->term_id;    
        ...
        foreach ($productsSeg as $products_) {
            ...
            foreach ($productPrices as $product) {
                if (!is_null($product['shop_id'])) {
                    $data_productos_ = [
                        'id' => $product['shop_id'],
                        'stock_quantity' => ((setBoolean($product['active']))?$product['stock']:0),
                        'regular_price' => ((setBoolean($product['active'])) ?$product['price']:0),
                        "status" => 'publish',
                        'sale_price' => ((setBoolean($product['active'])) ? $product['price'] : 0),
                        'stock_status' => ((setBoolean($product['active'])) ? ((intval($product['price']) > 0 && intval($product['stock']) > 0) ? "instock" : "outofstock") : "outofstock"),
                        'manage_stock' => true
                    ];
                    $data_productos_['sku'] = $product['sku'];
                    $data_productos["update"][] = $data_productos_;
                    if(setBoolean($product['active'])){$totalActives+=1;}
                }
            }
...
function woocommerceproductsrc($products,$access,$id=null,$country=null,$operation="update", $skus = null, $update = 'all'){
    global $access_;
    global $parent_id_new;
    global $catalogs_;
    $access_= $access;
